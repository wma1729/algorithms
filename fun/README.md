...
This is very similar to the previous approach though it done using *divide and conquer* and uses recursion. The minimum and the maximum items for the smaller subsets are calculated. The results from the smaller subsets are used to solve the bigger subsets until the whole set is solved.
```C++
static void
